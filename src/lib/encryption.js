...
import { letscryptExtension } from './constants';
...
export const ENCRYPTION_SIZE_LIMIT_IN_MEGA_BYTES = 250;
...
export async function generateKeyPair() {
  ...
  return {
    privateKey: `s53privatekey:${unarmor(generated.privateKey)}`,
    publicKey: `s53publickey:${unarmor(generated.publicKey)}`,
  };
}
...
export async function encryptText(text, publicKey) {
  try {
    ...
    return { data: `s53:${unarmored}`, error: null };
  } catch (error) {
    console.log(error);
...
function getEncryptedFileName(fileName) {
  ...
  return `${format(new Date(), 'yyyyMMdd_HHmmss_SSS')}${extension}.${letscryptExtension}`;
}
...
export async function decryptFiles(files, privateKey) {
  ...
  await asyncForEach(files, async file => {
    const { name, path, size } = file;
    if (!name.endsWith(letscryptExtension)) {
      decryptedFiles.push({
        name,
        path,
        size,
