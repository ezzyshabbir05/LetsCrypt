...
import { letscryptExtension } from '../../lib/constants';
...
function* handlePickEncryptedFilesPressed() {
  ...
  const encryptedFiles = files.filter(f => f.name.endsWith(letscryptExtension));
  yield put(fileActionCreators.setEncryptedFiles(encryptedFiles));

  if (encryptedFiles.length !== files.length) {
    yield put(toastActionCreators.setToast('Only pick files end with .s53'));
    return;
  }
}
...
function* handlePickOriginalFilesPressed() {
  const files = yield call(pickFiles);
  const originalFiles = files.filter(f => !f.name.endsWith(letscryptExtension));
